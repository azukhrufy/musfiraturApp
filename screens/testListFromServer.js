...
class testListFromServer extends Component {
    constructor(props) {
        super(props);
        // step 1 fetch json
        this.state = { 
      ...
      componentDidMount() {
        ...
        fetch('https://my-json-server.typicode.com/azukhrufy/testmPaketWisata/result')
    ...
    //step 5
      renderItemPaketWisata = item => {
        ...
        <TouchableOpacity style = { styles.item } activeOpacity = { 0.4 } onPress = {() => navigation.navigate('PaketHaji', { article: item })}>
            ...
            <View style={{ flex: 0 }}>
            <Image source={{ uri: item.fotopaketperjalanan }} style={styles.avatar} />
            </View>
            <View style={[styles.column, { flex: 2, paddingHorizontal: theme.sizes.padding / 4 }]}>
            <Text style={{ color: theme.colors.black, fontWeight: 'bold' }}>{item.nmpaketperjalanan}</Text>
...
export default testListFromServer;
